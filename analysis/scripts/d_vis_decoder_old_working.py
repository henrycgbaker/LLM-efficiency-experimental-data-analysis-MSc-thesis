...
def _plot_with_band(ax, raw_df, x_col, y_col, mean_df, mean_col, std_col,
    ...
    if ax in normalise_axes:
        ...
        lower      = mean_vals - std_vals
        upper      = mean_vals + std_vals
        # Keep multiplicative formatter, remove axis label change
...
def plot_decoder_temperature(
    ...
    for i, m_name in enumerate(models):
        ...
        for method in methods:
            ...
            # Rename greedy to multinomial, remove model from label
            ...
            _plot_with_band(
                ...
                plot_band=plot_band,
                plot_raw=plot_raw,
                label_mean=method_label
            )

    # Baseline as greedy, darker and labelled
    if add_baseline_energy:
        for m_name in models:
            ...
            ax.axhline(base, linestyle='--', color='black', alpha=1.0, label='greedy')

    # Title with model name and suffix
    model_str = models[0] if len(models) == 1 else ','.join(models)
    title_base = 'Energy-per-Token vs Decoder Temperature'
    title = f"{model_str} {title_base}"
    suffix = "\n(Normalised)" if normalise_axes else "\n(Absolute Values)"
    plt.title(title + suffix)

    # Legend without model names
    ax.legend(loc='best')
    plt.tight_layout()
    plt.show()
...
def plot_decoder_top_k(
    ...
    if add_baseline_energy:
        for m_name in models:
            ...
            ax.hlines(base, xmin, xmax, linestyle='--', color='black', alpha=1.0, label='greedy')

    # Title with model name
    model_str = models[0] if len(models) == 1 else ','.join(models)
    title_base = 'Energy-per-Token vs Top-k'
    title = f"{model_str} {title_base}"
    suffix = "\n(Normalised)" if normalise_axes else "\n(Absolute Values)"
    plt.title(title + suffix)

    # Legend: no model, show temperature legend
...
def plot_decoder_top_p(
    ...
    if add_baseline_energy:
        for m_name in models:
            ...
            base = 1.0 if norm_axes else stats.iloc[0]
            xmin, xmax = ax.get_xlim()
            ax.hlines(base, xmin, xmax, linestyle='--', color='black', alpha=1.0, label='greedy baseline')

    # Title with model name
    model_str = models[0] if len(models) == 1 else ','.join(models)
    title_base = 'Energy-per-Token vs Top-p'
    title = f"{model_str} {title_base}"
    suffix = "\n(Normalised)" if normalise_axes else "\n(Absolute Values)"
    plt.title(title + suffix)
